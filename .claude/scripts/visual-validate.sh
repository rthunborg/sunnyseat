  ...
  "messages": [
    {
      ...
      "content": [
        {
          "type": "text",
          "text": "You are a visual QA reviewer comparing a UI implementation against its design reference.\n\nImage 1 (REFERENCE): The approved Claude Design prototype rendering for screen '$SCREEN_ID', captured by Playwright.\nImage 2 (IMPLEMENTATION): A screenshot of the current implementation running on the dev server.\n\nCompare them and respond with EXACTLY one of two formats:\n\nIf the implementation is acceptable:\nPASS: [one sentence summary of match quality]\n\nIf the implementation has blocking issues:\nFAIL: [comma-separated list of specific issues]\n\nBlocking issues are: wrong layout structure, missing UI components, incorrect colour scheme, broken responsive behaviour, missing or incorrectly positioned UI chrome elements within the app itself, AND any app chrome element whose WIDTH or EXTENT differs substantially from the reference. Specifically FAIL when a bar, pill, slider, sheet, or panel stretches edge-to-edge across the full viewport while the reference shows it contained/inset, centred, or stopping short to clear an adjacent sidebar or panel (or the reverse), or when an element overlaps / slides under an adjacent panel that it clears in the reference. Treat a component that visibly spans a clearly different fraction of the screen than the reference (roughly a third or more difference in width or horizontal position) as a blocking LAYOUT difference, not as spacing.\n\nNon-blocking (do not fail for these): minor pixel-level spacing differences (a few px of padding or margin), placeholder text/images, loading states, hover states not visible in screenshot. Small padding/margin nudges are fine and the design defines the visual target rather than exact pixel measurements — but this leniency applies ONLY to small spacing. It does NOT excuse a component being a substantially different width, spanning a different portion of the screen, or sitting at a clearly different horizontal position than the reference; those are blocking layout differences (see above).\n\nSPECIAL HANDLING — DEVICE FRAME CHROME: The reference is rendered inside a simulated device frame from the Claude Design prototype: an iOS device frame on mobile (rounded corners, dynamic island, status bar showing '9:41' with signal/wifi/battery icons, home indicator pill at bottom) or a browser window frame on desktop (window controls, address bar, tab bar). The implementation will NOT have these frames — it is the actual web app. You MUST IGNORE: presence/absence of the iOS dynamic island, iOS status bar (clock, signal, wifi, battery), iOS home indicator, browser window controls, browser address bar, browser tabs, and the device-frame rounded outer corners. These are simulator artifacts in the reference, not part of the app being implemented.\n\nSPECIAL HANDLING FOR MAP-BASED SCREENS: If the screen contains a map (screen IDs containing 'map', or any screen where a map canvas is visible), the map canvas content itself is dynamic and will never match the reference exactly. You MUST IGNORE the following map-related differences: different street layouts, different tile imagery, different geographic content, different map labels, different map zoom level, different map center coordinates, different exact positions of venue pins on the map, and different number of venue pins visible. You MUST FOCUS ON: UI chrome overlaying the map (search bars, floating buttons, bottom sheets, navigation bars, time sliders), the overall warm/sand colour tint of the map area matching the reference, the presence and visual styling of venue pins (sunny vs shaded variants should look correct even if positioned differently), and the overall layout composition of non-map elements. A map screen where the underlying map shows different streets but all UI chrome, colours, and pin styling match the reference should PASS. Only FAIL a map screen if the UI chrome itself is wrong — wrong button positions, missing bottom sheet, wrong colours on overlays, missing search bar, etc.\n\nBe strict on app-level UI chrome (the chrome that lives inside the implementation). Be lenient on simulator device-frame chrome, map content, and dynamic data."
        },
        {
          "type": "image",
          "source": {
...
if echo "$VERDICT" | grep -q "^PASS"; then
  ...
  echo ""
  echo "Two failure shapes — handle them differently:"
  echo "  (a) IMPLEMENTATION DEFECT — the failure points at something this story is responsible for"
  echo "      (wrong colours, missing AC1-7 chrome, broken layout). Fix the code and re-run the gate."
  echo "  (b) SCOPE DRIFT IN REFERENCE — the failure points at UI that this story is not responsible for"
  echo "      (downstream-story features, partial implementations expected from later stories). The"
  echo "      reference PNG predates the current story's scope and the gate cannot pass until those"
  echo "      stories ship. In this case you MUST stop and request explicit confirmation from Rasmus"
  echo "      before treating the story as complete or transitioning sprint-status to 'review'. Do NOT"
  echo "      modify this script, the sprint-status hook, the reference PNG, or any other gate logic"
  echo "      to bypass the failure. Surface the verdict and the per-flag scope analysis to Rasmus and"
  echo "      wait for an explicit accept-with-rationale before proceeding."
  rm -f "$IMPL_SCREENSHOT"
  exit 1
fi
