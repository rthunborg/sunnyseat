# PostToolUse hook (Write|Edit): eslint --fix on changed nextjs-app source files.
# Input: Claude Code hook JSON on stdin. Exit 2 = feed remaining lint errors back to Claude.
...
try { $out = npx eslint $file --fix --quiet 2>&1; $code = $LASTEXITCODE } finally { Pop-Location }
if ($code -ne 0) {
  $tail = ($out | ForEach-Object { if ($_ -is [Management.Automation.ErrorRecord]) { $_.Exception.Message } else { "$_" } } |
    Select-Object -Last 15) -join "`n"
  [Console]::Error.WriteLine("eslint: unfixable problems remain in ${file}:`n$tail")
  exit 2
}
exit 0
