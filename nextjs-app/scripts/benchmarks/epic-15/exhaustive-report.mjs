...
if(!directory||!path.isAbsolute(directory))throw Error('Absolute run directory required');
const read=n=>JSON.parse(fs.readFileSync(path.join(directory,n),'utf8'));
const data=read('exact-reference.json'),environment=read('environment.json');
validateLaneCompletion(read('completion.json'),artifactHashes(directory,['exact-reference.json','environment.json']),['34','8','47']);
if(JSON.stringify(environment.sourceHashes)!==JSON.stringify(artifactHashes(process.cwd(),Object.keys(environment.sourceHashes))))throw Error('Source identity mismatch');
for(const cell of data.cells){
  if(cell.points.length!==500||cell.sampleCount!==500||cell.end-cell.start!==500||cell.points.some((p,i)=>p.t!==cell.start+i)||cell.observations.length!==3)throw Error('Incomplete exhaustive cell');
  const values=cell.points.map(p=>p.value);
  if(cell.bounds.lower!==Math.min(...values)||cell.bounds.upper!==Math.max(...values))throw Error('Extrema mismatch');
}
console.log(`# Exact finite-Date reference — Story 15.1
...
|---|---:|---:|---:|---:|---:|
${data.cells.map(c=>`| ${c.id} | ${c.casters} | ${c.sampleCount} | ${(c.wallMs.p50/1000).toFixed(3)} | ${c.exactPredicateTransitions.join(', ')} | ${c.extrapolation.wallHours.toFixed(2)} |`).join('\n')}
