...
const currentSources=artifactHashes(process.cwd(),Object.keys(env.sourceHashes));
const reporter='scripts/benchmarks/epic-15/window-report.mjs';
// Retain the exact report source captured by the timed run when a subsequent
// report-only fix is needed. Measurement sources must still match current.
if(currentSources[reporter]!==env.sourceHashes[reporter])currentSources[reporter]=artifactHashes(directory,['window-report-at-capture.mjs'])['window-report-at-capture.mjs'];
if(jsonHash(currentSources)!==jsonHash(env.sourceHashes))throw Error('Window source changed');
...
const expectedCost=['courtyard','narrow-street','rooftop-surviving'].flatMap(f=>dates.map(d=>`${f}/${d}`));
const costKeys=cost.cost.map(c=>`${c.fixture}/${c.date}`);
if(costKeys.length!==21||new Set(costKeys).size!==21||expectedCost.some(k=>!costKeys.includes(k))||cost.cost.some(c=>c.n!==5||c.observations.length!==5||c.observations.some(r=>!Number.isFinite(r.wallMs)||!Number.isFinite(r.cpuMs)||!Number.isInteger(r.samples))))throw Error('Incomplete daily computation evidence');
const season=Array.from({length:245},(_,i)=>new Date(Date.UTC(2026,2,1)+i*86400000).toISOString().slice(0,10));
if(cost.fullSeason.n!==1||cost.fullSeason.p95!==null||jsonHash(cost.fullSeason.dates.map(d=>d.date))!==jsonHash(season))throw Error('Incomplete full-season computation evidence');
if(jsonHash(analytic.map(a=>a.id))!==jsonHash(['duration-299','duration-300','duration-301','missed-shade-gap']))throw Error('Incomplete analytic matrix');
const fixed=n=>n===null?'not estimated':Number(n.toFixed(3));
...
fs.writeFileSync(path.join(directory,'report-generator.json'),JSON.stringify({sourceHashes:artifactHashes(process.cwd(),[reporter]),note:'Post-capture reporter handles null p95 for n<20 and validates computation/analytic cell coverage. Original captured reporter retained; raw measurements unchanged.'},null,2));
