...
const RECIPES = [
  ...
  // map-primary-offline: Story 7.3 captured the first implementation-driven
  // baseline (the offline shell — cached map background + "Ingen anslutning"
  // banner, no venue data; desktop keeps the real DesktopNavBar above the
  // banner). There is no prototype recipe for the offline shell, so re-baseline
  // via scripts/capture-offline-rebaseline.mjs against the running app.
  ...
  // map-selected-time-open / map-selected-time-closed: Story 12.14 selected-time
  // availability states are implementation-derived routes, not Claude prototype
  // recipes. See project-context.md and STATE-MAPPING.md; active PNG promotion is
  // deferred until explicit maintainer approval.
];
