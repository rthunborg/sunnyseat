...
describe('GET /api/venues', () => {
  ...
  it('returns venue data with sun exposure', async () => {
    ...
    expect(body.venues[0].venueName).toBe('Café Husaren');
    expect(body.venues[0].slug).toBe('cafe-husaren');
    expect(body.venues[0].neighborhood).toBe('Haga');
    expect(body.venues[0].currentSunStatus).toBe('Sunny');
  ...
  it('gracefully degrades when sun exposure calculation fails', async () => {
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.venues[0].currentSunStatus).toBe('Shaded');
  });
  ...
  it('sorts by sun status primary, distance secondary', async () => {
    ...
    expect(body.venues[0].distanceMeters).toBe(100);
    expect(body.venues[1].distanceMeters).toBe(500);
  });
