...
type StoryStep = VenueDaySeriesEntry & {
...
const SPECS: VenueSpec[] = [
  { id: 'low-partial', name: 'Lag partial', exposure: 40, gate: 'not_gated', status: 'Partial', directSunState: 'blocked', directSunReasons: ['geometry'], offset: 0 },
  { id: 'exact-fifty', name: 'Exakt femtio', exposure: 50, gate: 'not_gated', status: 'Partial', directSunState: 'blocked', directSunReasons: ['geometry'], offset: 0.001 },
  { id: 'over-fifty', name: 'Precis over', exposure: 51, gate: 'not_gated', status: 'Partial', directSunState: 'likely', directSunReasons: [], offset: 0.002 },
  { id: 'gated-high', name: 'Molngatad hog', exposure: 95, gate: 'gated', status: 'CloudObscured', directSunState: 'blocked', directSunReasons: ['cloud-obstruction'], offset: 0.003 },
  { id: 'unknown-high', name: 'Okant vader', exposure: 80, gate: 'unknown', status: 'Sunny', directSunState: 'unknown', directSunReasons: ['weather-unavailable'], offset: 0.004 },
];
...
function venue(spec: VenueSpec): StoryVenue {
  return {
    ...
    directSunState: spec.directSunState,
    directSunReasons: spec.directSunReasons,
    isPartner: false,
...
test.describe('Story 12.6 - deterministic public-sun pin journey', () => {
  test('[P0] renders the 40/50/51/gated/unknown matrix honestly on mobile and desktop', async ({ page }) => {
    ...
    const low = page.getByRole('button', { name: /Lag partial.*ingen direkt sol v.ntas vid vald tid/i });
    ...
    await expect(unknown).not.toContainText(/\d+%/);
    await expect(unknown.locator('[data-pin-icon="cloud"]')).toHaveCount(1);
  ...
  test('[P0] selection preserves the same amber semantic shape and marker root', async ({ page }) => {
    ...
    const pin = page.getByRole('button', { name: /Precis over.*direkt sol sannolik vid vald tid.*51/i });
