...
function Sidebar({
  ...
  const sunnyCount = filtered.filter(v => sunPctAt(v, hour) >= 0.45).length;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {isFavTab ? 'Dina favoriter' : 'Toppval nära dig'}
        </div>
        <div style={{
          ...
          {isFavTab
            ? `${sorted.length} ${sorted.length === 1 ? 'sparad plats' : 'sparade platser'}`
            : `${sunnyCount} soliga platser`}
        </div>
        {!isFavTab && (
...
function SideTab({ active, onClick, icon, label, locked }) {
  return (
      ...
      <div style={{ position: 'relative' }}>
        ...
        {locked && (
          <div style={{
            ...
            <Icon name="lock" size={7} color="#ffe088"/>
          </div>
        )}
      </div>
      <span style={{
