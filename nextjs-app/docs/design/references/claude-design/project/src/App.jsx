...
function App() {
  ...
  const [screen, setScreen] = useState(() => {
    return localStorage.getItem('sunny_screen') || 'onboarding';
  });
  ...
  const [modal, setModal] = useState(null); // 'datepicker' | 'feedback' | 'review' | 'notfound' | 'paymentFailed'
  ...
  useEffect(() => { localStorage.setItem('sunny_screen', screen); }, [screen]);
          ...
          {screen === 'map' && (
            <>
              ...
              <div style={{
                ...
                <FloatBtn onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }); setSelected(null); setSheetState('mid'); }} title="Återgå till min plats">
                  <Icon name="location" size={20} color="#735c00" fill="#735c00"/>
                </FloatBtn>
              ...
              <TabBar tab={tab} setTab={setTab} height={FOOTER_H}
                onNearToggle={() => { setTab('near'); setSheetState(s => s === 'peek' ? 'mid' : 'peek'); }}
                onFavToggle={() => {
                  setTab('fav');
...
function TabBar({ tab, setTab, height = 52, onNearToggle, onFavToggle }) {
  return (
    <div style={{
