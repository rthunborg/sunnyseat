...
function BottomSheet({ venues, hour, sortMode, setSortMode, onVenueSelect, state, setState, footerH = 40, tab = 'near', favorites, onToggleFavorite }) {
  ...
  const sunnyCount = sorted.filter(v => sunPctAt(v, hour) >= 0.45).length;
  const headerTitle = isFavTab
    ? `${sorted.length} ${sorted.length === 1 ? 'favorit' : 'favoriter'}`
    : `${sunnyCount} soliga platser nära dig`;
  const headerSub = isFavTab
    ? (sorted.length === 0 ? 'Tryck på hjärtat på en plats för att spara' : 'Sparade platser · uppdaterade just nu')
    : 'Uppdaterad just nu · Göteborg';
    ...
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={cycle} style={{
          ...
          transform: state === 'full' ? 'rotate(180deg)' : 'rotate(0deg)',
          transition: 'transform 0.3s',
        }}>
          <Icon name="chevronDown" size={15} color="#4d4635"/>
        </button>
      </div>
