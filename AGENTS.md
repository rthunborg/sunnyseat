...
> **Working directory:** The active repository root is `C:\DEV\sunnyseat\`; this is **not** the Next.js app root. The application root is `C:\DEV\sunnyseat\nextjs-app\`. Git operations and planning-doc reads happen from the repository root. All `npm`/`npx` app commands must run from the application root (`nextjs-app/` when starting at the repository root).

- Frontend: Next.js 16.3.3 App Router, TypeScript strict, Tailwind CSS v4 CSS-first `@theme`, shadcn/ui v4, MapLibre GL JS 6.4.1, TanStack Query 5.x, Motion 12.x (`motion/react`), `@use-gesture/react`, `next-intl`, Serwist, `date-fns-tz`
