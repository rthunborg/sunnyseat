...
- **The server emits a whole-day gated `sunDaySeries` (one Epic-10-gated entry per `PLANNER_STEP_MINUTES` step); the client derives every time-dependent surface from it and NEVER re-gates.** The single client seam is the pure, client-safe `deriveVenueSunAtMinutes` (`lib/utils/venue-day-series.ts`), applied via `applyDaySeriesDerivation` in `MapView.tsx`. It feeds marker %, public pin state, quick-info figures, "Mest sol" ordering, and the obscured sky sub-line — so a **settled same-date time scrub derives all time-dependent UI offline-from-network and issues ZERO venue requests** (R-001, the epic headline). The Epic-10 weather gate stays authoritative **server-side**; the client only READS the per-step `currentSunStatus` / `sunExposurePercent` / `weatherGateState` / gated sky phrase. Public sunny is `sunExposurePercent > 50 && weatherGateState !== 'gated'`; all grey map pins are percentage-free cloud pins, while `CloudObscured` remains diagnostic copy outside the pin.
...
- **One public-sunny predicate:** `isVenuePubliclySunny` in `lib/utils/public-sun.ts` is the shared source for amber vs grey public presentation and sorting. Public sunny means `sunExposurePercent > 50 && weatherGateState !== 'gated'`; raw `VenueSunStatus` is diagnostic and must not independently decide public colour.
