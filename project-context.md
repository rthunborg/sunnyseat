...
| map-selected-time-closed   | `/favoriter?_state=map-selected-time-closed&_time=09:00`   | mobile   | Story 12.14 selected-instant closed reference — dev forcing treats seeded venue id `1` as a saved favourite; the `11:00-22:00` venue is retained as a labelled closed row and excluded from pins at 09:00. Implementation-derived active PNG requires maintainer approval before promotion. |
| map-selected-time-closed   | `/favoriter?_state=map-selected-time-closed&_time=09:00`   | desktop  | Same retained closed favourite reference on desktop; seeded venue id `1` remains inspectable but unpinned at 09:00. Implementation-derived active PNG requires maintainer approval before promotion. |
