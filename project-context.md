...
>
> Last updated: 2026-08-07 (Epic 12 implementation-context refresh; Epic 9–11 ratified conventions remain, Epic 12 proven conventions promoted below)
>
...
- **The real data path is LIVE in production** (cutover 2026-06-29). The full stack — frontend, real sun/shadow engine, Met.no weather, Supabase venue store + feedback/review persistence — runs against live data. Epics 1, 2, 3, and 7–11 are shipped history; Epic 12 implementation conventions below are current where promoted, while final review/retro/approval status remains owned by sprint status.
...
- **Epic 12 ("Real-Venue Launch Readiness", stories 12.1–12.14) has moved from planning substrate to implemented conventions across the active sprint.** It adds provider-neutral hours governance, persisted deterministic sun-geometry/day-series reads, weather snapshots, shared live public-venue identity/visibility, public-sunny presentation, internal-only confidence, Supabase Storage media renditions, bounded detail prefetch, a first-run coach guide, selected-instant availability, and a dev-only venue editor. Some story rows remain `review`, so keep remaining operational evidence and final approval separate from durable implementation rules.
...
- **Public requests read persisted deterministic geometry and read-time weather snapshots.** Missing exact current coverage fails closed; do not silently fall back to stale persisted coverage or live recomputation in public list/detail routes. The precompute/snapshot jobs and RPCs are service-role paths, not public API behavior.
