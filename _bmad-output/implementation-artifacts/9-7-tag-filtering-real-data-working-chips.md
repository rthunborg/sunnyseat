...
_(Thin Tier-A epic-mode review, R=1: Acceptance Auditor lens + dedicated Security review. Blind/Edge lenses intentionally not run in Tier A. Security: 0 findings, all render sinks / SQL / RLS clean. Triaged 2026-07-01.)_

- [x] **[Review][Patch][Med][RESOLVED] Nav's `useVenueSearch` query key diverges from MapView's — the "zero new requests / TanStack de-dupe during a slider drag" fetch-hygiene invariant (Story 9.4 spine, this story's Dev Notes §"Constraints carried from Epic 9 retro-notes") is not strictly guaranteed.** `DesktopNavBar.tsx` (diff L101-106) issues `useVenueSearch({ lat, lng, radiusKm: SEARCH_RADIUS_KM, ...plannerTime.plannerQuery })` — the **un-deferred** `plannerTime.plannerQuery` and **no `enabled` gate**. `MapView.tsx:195-201` issues `useVenueSearch({ lat, lng, radiusKm, enabled: coordsSettled, ...deferredPlanner })` (`deferredPlanner = useDeferredValue(plannerTime.plannerQuery)`, `coordsSettled = status==='success'||'fallback'`). During a rapid time-slider drag the nav's key jumps to the new planner value while MapView holds the previous deferred key → keys diverge → the nav's chip-driving query can issue an extra fetch the story explicitly claimed never happens, and won't de-dupe against MapView's; the missing `enabled` gate also lets the nav fire before geolocation settles. Self-heals on settle and does not change results (scoped Med, not High). Fix: align the nav-side key + gate with MapView's — feed the SAME deferred planner value (`useDeferredValue(plannerTime.plannerQuery)`) and add `enabled: coordsSettled` (same `geolocation.status === 'success' || 'fallback'` derivation) so the two keys are byte-identical and TanStack collapses them into one request, restoring the drag/settle hygiene.
  - **RESOLVED (2026-07-01):** `DesktopNavBar.tsx` now derives `coordsSettled = geolocation.status === 'success' || 'fallback'` and `deferredPlanner = useDeferredValue(plannerTime.plannerQuery)` — the exact same two derivations MapView.tsx:178-179,194 uses — and issues `useVenueSearch({ lat, lng, radiusKm: SEARCH_RADIUS_KM, enabled: coordsSettled, ...deferredPlanner })`. Both call sites now share identical `SEARCH_RADIUS_KM` (1.5), `useGeolocation()` coords/status, and `useTimeContext().plannerQuery` sources, so the resulting query keys (`{lat, lng, q, radiusKm, ...planner}`) are byte-identical and the nav's fetch de-dupes against MapView's; the `enabled` gate stops the nav firing before geolocation settles. Chip union still populates from `venueQuery.data?.venues`. Added a de-dupe-invariant test to `DesktopNavBar.test.tsx` asserting the nav calls `useVenueSearch` with `{lat, lng, radiusKm, enabled: false (idle), date, time}` (mirrors MapView's shape/gate). Gate: tsc 0 · eslint 0 · vitest 102 files / 873 tests all green.

- **[Review][Defer][Low] Empty-state can render the loading skeleton instead of the "nothing matches" copy during a concurrent background refetch.** Both `<VenueList>` call sites pass `isLoading={venueQuery.isFetching && listVenues.length === 0}` (`MapView.tsx:926,970`) and `VenueList.tsx:55` returns the loading skeleton BEFORE the `sortedVenues.length === 0` empty branch (`:70`). A 0-match tag filter while the underlying query is `isFetching` in the background (e.g. a `keepPreviousData` planner-change refetch) shows the 3-card skeleton, not `venue.list.empty`. Steady-state (query idle) renders the empty copy correctly and the MapView tag-filter test asserts it; transient edge on pre-existing `isLoading` wiring reused unchanged. Deferred (see `deferred-work.md`).
