...
| 2026-06-18 | Code review **Round 2 of 3** (bmad-code-review, 3 parallel layers, Opus 4.8). All 5 Round 1 fixes verified present & correct in live source; all 4 ACs re-verified satisfied; seed byte-identity re-confirmed; no regressions. Triage: 0 patch, 0 decision-needed, 2 deferred (1 conditional Supabase by-slug/`venueSlug` asymmetry; 1 → Story 8.5 opt-in-path pre-cutover verification), 9 dismissed. Clean re-review. Status review → done. |
...
**Round 2 of 3** — bmad-code-review re-review (3 parallel layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor), 2026-06-18, Opus 4.8. Diff vs baseline `62f6438` (commit `aceb4c9` — the full Story 8.2 change set incl. the Round 1 fixes; payload `validation/8-2-review-payload-r2.diff`). **Follow-up review verifying the Round 1 fixes and hunting regressions.**

**Round 1 fixes verified present & correct** (Acceptance Auditor, full source access): all 5 R1 Patch items (P1 enum coercion, P2 identity/coordinate guards, P3 by-slug error-branch test, P4 sibling-fetcher `HttpError` migration, P5 `httpStatusFromError` narrowing) are landed in the live source, not just claimed in the changelog. All 4 ACs re-verified SATISFIED; seed byte-identity re-verified field-by-field across all 7 venues; scope guardrails clean (no sun/weather/confidence math change, no anon/public RLS, no `types.ts` regen, no persistence-wiring change, no live-Supabase CI dependency). **No regressions found. 0 new blocking (Critical/High-in-CI) findings, 0 patch-worthy findings.**

_Patch findings (Round 2):_ none.

_Deferred findings (Round 2):_

- [x] [Review][Defer] Supabase by-slug lookup matches `slug` only, not `venueSlug` — asymmetric with the in-memory path and the old route's dual `slug || venueSlug` match [`nextjs-app/lib/services/venue-store.ts`] — deferred, **conditional** (Target: None — only triggers if a `venue_slug` / alias / slug-history column is ever added to `public.venues`). Behaviorally equivalent today: the `venues` table has a single `slug` column and `fromVenueRow` derives `venueSlug = slug`, so no request can match `venueSlug` without also matching `slug`. Raised **High** by the two context-blind layers; re-confirmed equivalent by the full-context Acceptance Auditor (also dismissed as noise in Round 1). Honor the alternate identifier in the by-slug query if one is ever introduced.
- [x] [Review][Defer] Opt-in Supabase venue-store path — pre-live-cutover verification (uniqueness-key alignment + query-contract test coverage) [`nextjs-app/app/api/venues/route.ts`, `nextjs-app/lib/services/venue-store.ts`, `nextjs-app/test/unit/services/venue-store.test.ts`] — deferred, Target: **Story 8.5** (Production Config & Security Hardening — the live cutover). (a) `validateVenueUniqueness` dedupes on `id` + rounded coordinates while the SQL contract enforces unique `id` + `slug` — neither enforces the other, so two live venues sharing rounded coordinates would 500 the list route. (b) The mocked Supabase tests ignore the args passed to `.select(VENUE_SELECT_COLUMNS)` / `.eq('slug', …)`, so a column/filter typo in the snake_case query contract surfaces only against the real table. Both bite only with `SUNNYSEAT_VENUE_STORE=supabase` (off in CI); verify when 8.5 enables the live read.

_Dismissed as noise / neutralized-by-context (Round 2):_ in-memory `getVenues()` detail-strip symmetry (no current leak — `VENUE_FIXTURE` is base-only by design and pinned by the "omits detail block" test; also dismissed R1); `coerceSunStatus` case-sensitivity (exact-case match correctly mirrors the SQL `CHECK`; case-insensitive matching would mask contract violations; coerce-to-`NoSun` is the accepted R1-P1 fail-safe); `coerceSkyCondition` admits `'unavailable'` (verified valid — `skyCondition?: string` in `api.ts` documents `'unavailable'`); `numberOr` confidence/sun-exposure range (guarded by the SQL `CHECK (… between 0 and 100)`; non-crashing, below the R1-P2 bar — and the file's "downstream sanitizes" comment is correctly scoped to sunWindow/thumbnail/reasons, which `normalizeVenueForResponse` does handle); `on conflict (id)` re-seed slug-collision (idempotent for the file as-written); SQL `sky_condition` unused `'unavailable'` enum + Shaded-venue confidence/exposure pairing (cosmetic; TEMP carriers superseded by 8.3); `formatBadValue` `JSON.stringify` on a BigInt id (unreachable — `id` is a `text` column); `httpStatusFromError` negative-int comment wording (cosmetic, zero behavior); `isClientHttpError` status-0 boundary (treating an opaque status-0 as retryable is acceptable).

**Round 2 outcome:** clean re-review — all Round 1 fixes verified present & correct, all 4 ACs satisfied, no regressions, **0 patch / 0 decision-needed, 2 deferred (1 conditional, 1 → Story 8.5), 9 dismissed.** Status: review → done.
