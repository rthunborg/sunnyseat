...
- 2026-07-05 — Desktop date-change e2e fix (`epic-11-scrub-zero-fetch.spec.ts`, AC3). The date-change test failed only under `--project=desktop` (mobile passed) because the selector `getByTestId('planner-date-next').first()` resolves the DOM-order-first control, which is the MOBILE panel's button (rendered first in `MapView.tsx`, `lg:hidden` on the desktop viewport) — a hidden element the click could never reach; on mobile the desktop panel is `hidden` so `.first()` coincidentally grabbed the visible one. The desktop `NextDayButton` itself is genuinely rendered and visible (probe confirmed a real desktop user sees/clicks it at ~(528, 630); `boundingBox` non-null, `isVisible()` true) — this was a spec selector bug, not a product defect, so no component change was needed. Fixed the selector to `.filter({ visible: true })`, which resolves exactly one visible instance per breakpoint (verified: 2 `planner-date-next` in the DOM, 1 visible on each of desktop/mobile). No assertions weakened. Result: `epic-11-scrub-zero-fetch.spec.ts` now 4/4 green (both tests × both `desktop` + `mobile` projects); full `vitest run` 139 files / 1288 passed / 0 failures (baseline match, no regression).
...
- [x] [Review][Patch][Med] Network-only favourite rows bypass the day-series derivation seam (partial AC1 miss) [nextjs-app/components/custom/map/MapView.tsx:433] — `applyDaySeriesDerivation` is applied only to `rawVenues` (the Närmast list cache). `favouriteVenueRows` tops up with `networkFavouriteRows` (`MapView.tsx:413-415`, the raw `favouriteVenueQuery.data.venues`) which are merged in at `MapView.tsx:433-437` WITHOUT passing through the derivation. `useFavouriteVenues` hits the same `/api/venues` real-engine path so those DTOs carry `sunDaySeries`, yet a favourite present ONLY in the favourites payload (an out-of-radius favourite or a cold `/favoriter` deep link) renders the server's single-instant `currentSunStatus`/`sunExposurePercent` and does NOT update on a same-date time scrub — its card and its "Mest sol" rank stay frozen. AC1 names "both venue lists" and the story client-seam note (lines 118-119) names the favourites list explicitly. Fix: map `networkFavouriteRows` through `applyDaySeriesDerivation(venue, plannerTime.selectedMinutes)` before the top-up merge (mirroring `rawVenues`), or apply the derivation once at the `favouriteVenueQuery.data.venues` seam. Source: Acceptance Auditor (primary).
  - **Resolved 2026-07-04:** `networkFavouriteRows` is now a `useMemo` that maps `favouriteVenueQuery.data.venues` through `applyDaySeriesDerivation(venue, plannerTime.selectedMinutes)` at the `favouriteVenueQuery.data.venues` seam (keyed on `[favouriteVenueQuery.data?.venues, plannerTime.selectedMinutes]`), mirroring the `rawVenues` derivation. Out-of-radius / cold-deep-link favourites (present only in the favourites payload) now read the per-step gated `currentSunStatus`/`sunExposurePercent` and track a same-date time scrub — both the card figures and the "Mest sol" rank. `EMPTY_VENUES` short-circuit preserved (no derivation churn when empty). Gates: `tsc --noEmit` 0 errors; `eslint MapView.tsx` 0 errors (3 pre-existing warnings only); full `vitest run` 130 files / 1175 passed / 0 failures (MapView + FavouritesList + useFavouriteVenues suites all green).
