...
**Round 2 of 3** — reviewed 2026-06-14 via `bmad-code-review` (Blind Hunter + Edge Case Hunter + Acceptance Auditor) over the merged Story 3.4 change set (`git diff 2dfe1dc..f8fecbb`, nextjs-app paths only). This is a re-review of already-`done`/merged work (Epic 3 shipped via PR #10). Acceptance Auditor verdict: all 7 ACs SATISFIED, and Round 1 patches P1–P11 verified landed in code and regression-free (re-ran `tsc` 0, routing-boundary/messages-parity/RouteOverlay/MapView suites green). After orchestrator code-verification of every raised claim: **0 decision-needed, 2 patch (both LOW), 5 defer (1 MEDIUM / 4 LOW), 13 dismissed**. Dismissed set was confirmed against source: BH "canRequestVenueDetail undefined" (symbol exists; not-found notice is test-pinned for real unknown slugs), BH "accessible confidence dropped when visible" (impossible — `confidence-display.ts` always sets `accessibleText` for exact/approximate kinds), BH "synthetic-confidence asymmetry in `handleRouteSelectedVenue`" (the synthetic `fallbackVenueFromSlug` venue only flows through `detailFallback`→`detailRouteVenue`; the quick-info path uses `venueDtosForMap`-derived `selectedQuickInfoVenue`), BH "e2e confidence assertion is a fixture time-bomb" (`weather-freshness-fixture.ts` uses `bucketTimestamp(now)`, always fresh → always the `exact` kind), the non-memoized `detailErrorNotice` re-announce theory (props are value-stable; no remount/flip outside the `isFetching`-bracketed error transition), the required-favourite-label "raw-key leak" (intended Task 6.4 hardening, TS-enforced, MapView always passes localized values), plus the Acceptance Auditor's own three no-action notes (list-error suppression is the deliberate P3 anti-stacking tradeoff; confidence visible/accessible duplication is latent-not-bug; completion-note phrasing verified accurate).

- [x] [Review][Patch] R2-P1 (LOW): Whitespace-only `?venue=%20` deep link shows no not-found notice and strands a dead `?venue=` param — `useVenueDetail` trims the slug to `""` and sets `enabled:false`, so `venueDetailQuery.isError` never becomes true and `detailErrorNotice` stays null; `detailFallback` also returns null (no list/favourite match, not fetching). AC #2's invalid-slug not-found path is bypassed for blank/whitespace slugs (niche: only reachable via a hand-crafted URL) [nextjs-app/components/custom/map/MapView.tsx:699-713, nextjs-app/hooks/queries/useVenueDetail.ts:31,102] — **FIXED 2026-06-14:** `detailErrorNotice` now treats a present-but-blank slug as not-found (back-to-map clears the param); pinned by new MapView test "renders a localized not-found state for a present-but-blank venue slug".
- [x] [Review][Patch] R2-P2 (LOW): Route overlay drops the "confidence unavailable" screen-reader text when confidence is hidden but uncertainty is present — `routeConfidenceLabel` gates `accessibleParts` on `confidenceDisplay.visibleText`, so the `hidden`-kind `accessibleText` ("Säkerhet saknas") is discarded; `VenueQuickInfo` emits that same text via an `sr-only` span, so SR users get a parity gap across surfaces built on the same Story 3.0.6 helper [nextjs-app/components/custom/map/MapView.tsx:1215-1218] — **FIXED 2026-06-14:** `routeConfidenceLabel` now includes `confidenceDisplay.accessibleText` unconditionally, so a hidden-confidence-with-uncertainty row announces "Säkerhet saknas" to screen readers; pinned by new MapView test "announces confidence unavailable to screen readers".
- [x] [Review][Defer] R2-D1 (MEDIUM): A rejected review photo (oversized / non-image / 0-byte / over-long name) is silently dropped — `isSafeOptionalPhoto` false → `setPhoto(undefined)` writes nothing to the always-mounted `role="status"` region, so no user (sighted or SR) gets feedback that the pick was refused [nextjs-app/components/composed/feedback/ReviewForm.tsx:193-197,219-229] — deferred, pre-existing from Story 3.3 (this round only added the announcement region, not the rejection branch); fix needs a localized rejection string. *(Target: Story 8.4 — Feedback & Review Persistence Enablement)*
- [x] [Review][Defer] R2-D2 (LOW): `isVenueNotFoundError` couples 404 detection to the human-readable error string via `/failed:\s404\b/i` — works for the only current message format (`Venue detail failed: 404 Not Found`) and mirrors the established 4xx convention, but a future error-format change silently breaks the not-found branch [nextjs-app/hooks/queries/venue-query-options.ts:427] — deferred; robust fix is to carry a numeric status on the thrown error (cross-layer, belongs with the real API contract). *(Target: Story 8.2 — Real Venue Store & API)*
- [x] [Review][Defer] R2-D3 (LOW): `messages-parity` ICU test pins argument NAMES only, not formatter types/styles — an sv/en drift like `{minutes, number, integer}` vs `{minutes}` passes [nextjs-app/test/unit/messages-parity.test.ts] — deferred; the test already catches the primary drop-an-argument risk, formatter-style parity is an enhancement. *(Target: None — conditional, only triggers if an ICU formatter-style sv/en drift is suspected)*
- [x] [Review][Defer] R2-D4 (LOW): `routing-boundary` test is brittle — `.toEqual([...exact consumer paths])` forces an unrelated future story to edit it on any legitimate new routing consumer, and the raw-text `includes('window.open(')` / native-URL pattern scans count matches inside comments or string literals [nextjs-app/test/unit/routing-boundary.test.ts] — deferred; it is a deliberate boundary pin, but the matching could false-trip. *(Target: None — conditional, only triggers if a legitimate new routing consumer is blocked by the assertion)*
- [x] [Review][Defer] R2-D5 (LOW): `VenueDetailError` has no internal invariant for actionless prop combos — `{isNotFound:true, showBack:false}` would render an alert with neither a retry nor a back control; currently unreachable (the sole call site always pairs `isNotFound:true` with `showBack:true`) but a future refactor of the precedence IIFE could strand the user [nextjs-app/components/custom/map/MapView.tsx:1380-1421] — deferred; add a defensive `showBack || !isNotFound` guard (or always render Back) when the component is next touched. *(Target: None — conditional, only triggers if the detail-error precedence logic is refactored)*
...
| 2026-06-14 | Claude (review) | Code review Round 2 of 3 (re-review of merged Story 3.4, `2dfe1dc..f8fecbb`): all 7 ACs SATISFIED, Round 1 P1–P11 verified landed/regression-free. 0 decision-needed, 2 patch (LOW), 5 defer, 13 dismissed (false positives confirmed against `confidence-display.ts`, `weather-freshness-fixture.ts`, and the selection derivations). Defers recorded in deferred-work.md (R2-D1→8.4, R2-D2→8.2, R2-D3/D4/D5 conditional). |
| 2026-06-14 | Claude (review) | Both Round 2 patches applied and pinned: R2-P1 (blank/whitespace `?venue=` slug → localized not-found with back-to-map) and R2-P2 (route overlay announces "Säkerhet saknas" to screen readers when confidence is hidden but uncertainty shows). Gates green: tsc 0, eslint 0, Vitest 64 files / 527 tests (+2 new), focused Playwright 16/16 (visit-loop mobile+desktop, axe a11y 8/8), all five visual validations PASS with no rebaseline. Status remains done. |
