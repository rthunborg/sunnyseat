...
_Thin Tier-A code review (2026-07-05). Sources: Acceptance Auditor@primary (0 Critical / 0 High / 3 Med / 4 Low); dedicated security pass (0 findings). Blind/Edge lenses reserved for the epic-level integration review (not run here — not failed layers). After triage: 2 Decision, 0 Patch, 1 Defer, 4 Dismissed._

- [x] [Review][Decision][Med] Mobile chip row hidden in favourites mode but the desktop chip strip is not — AC1 "identical on both breakpoints" asymmetry — In `MapView.tsx` the mobile `MobileTagChips` row is gated `mobileSheetState !== 'peek' && listMode !== 'favourites'`, while `DesktopNavBar`'s `TagChipStrip` renders unconditionally whenever `allTags.length > 0` (no favourites gate, `DesktopNavBar.tsx:40,64-79`). With favourites open a desktop user still has chips and can toggle a tag (which filters the shared `filterVenuesByTags` pins), while a mobile user has no chip affordance to clear a residual active tag — the two breakpoints diverge in favourites view. Dev rationalized "favourites are intentionally unfiltered," but that scope isn't applied symmetrically. Recommended: fix: hide the desktop `TagChipStrip` when `listMode === 'favourites'` (mirror the mobile gate) so both breakpoints scope chips to the Närmast list identically — the smaller, safer change than re-enabling filtering on the favourites surface. **[RESOLVED 2026-07-05: `DesktopNavBar` now reads `usePathname()` and gates the `TagChipStrip` render on `!isFavouritesRoute && allTags.length > 0`, mirroring the mobile `listMode !== 'favourites'` gate. On desktop `listMode === 'favourites'` is equivalent to the favourites route (the nearest/favourites toggle pushes `/favoriter` and MapView syncs `desktopListMode` to `isFavouritesRoute`), so the pathname is the shared signal this sibling component reads. The `retainTags` orphan-prune effect still runs while favourites is open, so a residual active tag can never strand the shared surfaces. Two new `DesktopNavBar.test.tsx` tests pin the gate (strip shown in nearest mode `/`; hidden on `/favoriter` + nested `/favoriter/:slug`), pathname made drivable via the `next-intl/navigation` mock without weakening any existing test. tsc/eslint clean; vitest 1288 pass.]**
- [x] [Review][Decision][Med] ~~DISMISSED (won't-fix, auto-resolved per triage recommendation — epic mode)~~ "1:1 finger tracking" (AC3) is suppressed under `prefers-reduced-motion`, and no test pins the 1:1 translate on either path — In `MobileBottomSheet.tsx` the live drag translate only applies `if (active && !shouldReduceMotion) setDragY(...)`; under reduced motion the sheet does not track the finger and only jumps to the resolved snap on release. AC3 states 1:1 tracking unconditionally, and no unit test asserts `dragY`/`y` follows `my` (the e2e asserts only terminal `data-state`, never intermediate position). Recommended: dismiss: jump-on-release under reduced motion is a legitimate, standard reduced-motion accommodation and AC3's "1:1" describes the default animated path (the release still lands the correct snap); keep the reduced-motion behaviour as-is. Optionally add a follow-up unit assertion that `dragY` tracks `my` on the default path — captured as the Deferred item below, not a blocker.
- [x] [Review][Defer][Med] AC4 arrow/overflow reachability proven only with jsdom-mocked scroll metrics; no real-overflow integration proof [nextjs-app/test/components/DesktopNavBar.test.tsx] — Every AC4 test drives `Object.defineProperty(strip, 'scrollWidth'/'clientWidth'/'scrollLeft', …)` + a synthetic `scroll` event because jsdom reports 0 for all scroll geometry; no e2e renders the desktop nav with a genuinely overflowing chip set and asserts a real arrow click reveals a previously-clipped chip. Implementation is correct and unit-covered; the gap is coverage adequacy on "all tags reachable at any viewport width," warranted by the epic's anti-"shipped-but-insufficient" thesis and the original real-overflow-clip bug. Deferred as test-hardening (a real-browser overflow e2e), not a code defect — pre-existing jsdom limitation, not introduced by this change.
