...
- [x] [Review][Patch][Low] AC3 edit not confined to `calcCloudCertainty` — large whitespace-only reindent churn (tabs→spaces) in the same file obscures the byte-identical guard [nextjs-app/lib/solar/confidence-calculator.ts:174-404] — The diff reformats leading indentation across UNTOUCHED functions/blocks: the import block (~L174-188), the `calcBuildingDataQuality`/`applyConfidenceCaps`/`identifyQualityIssues` call sites in `calculateConfidenceFactors` (~L196-251), and the whole bodies of `calcBuildingDataQuality` (~L259-278), `applyConfidenceCaps` (~L328-363), and `identifyQualityIssues` (~L365-404). Runtime logic of these is verified unchanged (no-weather `else` branch + caps chain behaviourally identical; the pinned `no-weather overallConfidence === 0.6` regression test passes), so the AC's byte-identical *behaviour* holds — but the cosmetic noise makes the real byte-identical guarantee impossible to eyeball and risks masking a future accidental logic edit in these blocks. Fix: revert the cosmetic reindentation so the diff shows ONLY the `calcCloudCertainty` + `cloudConfidenceFactor` + `CLOUD_CONFIDENCE_FLOOR` additions. (Corroborated by the security review, which independently flagged "the bulk of the hunk is whitespace/reindent noise".)
  - **RESOLVED (2026-07-03):** Root cause was NOT tabs→spaces reindentation but a line-ending artifact: the pre-story parent blob stored mixed EOLs (199 CRLF + 70 lone LF); the dev's editor rewrote the touched blocks to uniform CRLF, so 70 previously-LF lines flipped LF→CRLF and rendered as phantom "reindent" churn even though every visible byte was identical. Fix: reconstructed `confidence-calculator.ts` from the parent raw blob, applying ONLY the two AC3 logic edits (the `cloudFactor` line + comment in `calcCloudCertainty`, and the new `cloudConfidenceFactor` fn + `CLOUD_CONFIDENCE_FLOOR` const) and preserving every untouched line's original EOL. Verified: LF-normalized content is byte-identical to the prior story-committed version (no code changed); `git diff 24c28eb^ -- lib/solar/confidence-calculator.ts` (parent-of-story → working tree, i.e. what a reviewer sees once re-committed) now shows a SINGLE clean hunk = only the three AC3 additions, zero whitespace churn; the byte-identical no-weather guard is again eyeball-verifiable. Gate re-run green: `tsc` 0 errors, `eslint` 0 errors on the file, full `vitest run` 112 files / 993 tests all pass (incl. the pinned `no-weather overallConfidence === 0.6` regression).
- [x] [Review][Defer][Low] Latent DTO contract inconsistency: legacy `SunExposureResult.weatherData.cloudCover` widened to optional but public `WeatherDataDto.cloudCover` stays required `number` [nextjs-app/lib/types/api.ts:284] — deferred, pre-existing. `sun-exposure-service.ts` now types `weatherData.cloudCover?: number` and assigns the now-`number | undefined` value, but the public DTO `WeatherDataDto.cloudCover` (`lib/types/api.ts:284`, embedded in `VenueSunExposureResponse`) stays required `number`. No route wires `calculateSunExposure` → `VenueSunExposureResponse` today (grep over `app/**` finds no consumer), so tsc stays green (0 errors) and no runtime path is affected — consistent with the story's own note that this is a "legacy service, not on the live venues route." Dormant now; would surface an `undefined`-into-required-`number` mismatch only if that DTO is re-wired to a route.
