...
_Triage of the 2026-07-05 thin (Tier-A) code review — auditor lens (`primary`, 5 Low/observational, verdict Approve) + dedicated security review (0 findings). Blind/edge lenses reserved for the epic-level integration review (not run here). Verdict: **Approve** — no Decision or Patch findings; 2 genuine-but-minor Lows deferred; 3 dismissed as noise._

- [x] [Review][Defer][Low] AC3 secondary recenter site (OnboardingGate grant flyTo) not made viewport-aware [nextjs-app/components/custom/onboarding/OnboardingGate.tsx:215-224] — deferred, documented scope call. AC3's literal target (the recenter button) is fully handled; the grant path is a sibling of `MapView` with no access to sheet state and dismisses to the default `mid` snap where raw-coord landing is acceptable. AC3's Then clause is broader than the button, so this leaves one geolocation-driven camera move non-viewport-aware — genuine-but-minor, worth tracking for a follow-up.
- [x] [Review][Defer][Low] `MOBILE_TOP_BAR_COVER = 72` is a hand-picked constant with no mirrored CSS token [nextjs-app/lib/utils/recenter-padding.ts:60] — deferred, maintainability. Unlike every other padding value (120/320/560/340/390, all mirroring `--size-*` tokens), 72 is an eyeballed magnitude for the top-bar row the story deliberately left soft ("small `top` if it materially covers"). Within spec latitude, but could drift if the mobile search-bar height changes.

_Dismissed as noise (kept for the record — not persisted as actionable):_
- [Low] `collapsed` safe-area allowance `44 + 24 = 68` is an approximation of a device-only `env(safe-area-inset-bottom)` value — DISMISSED: documented JS limitation (`env()` is unreadable in JS), collapsed is a minor snap, and real-device framing verification is already explicitly owned by Story 11.8. Acceptable simplification, not a defect.
- [Low] AC1 legibility + axe-AA-green asserted by narrative/structural tests, not a diff-visible passing gate — DISMISSED: correct-by-design for a design-gate story (outcome-asserting tests + human visual-validation checkpoint are the intended gate; lowering a decorative overlay can only help contrast). The auditor itself states "No violation."
- [Low] Raw `rgba(217,119,6,…)` halo-gradient stops remain in `UserPin.tsx` (same colour value as the tokenized `#d97706`, a few lines away) — DISMISSED: preference/consistency nit. R-016 targets the `#d97706` hex literal (removed; source-guard test passes); raw rgba gradient stops match the established `globals.css` convention (`--gradient-map-overlay`, `--gradient-sun-burst-*`, and other gradients/shadows all keep raw rgba stops with no token alias). Not a defect.