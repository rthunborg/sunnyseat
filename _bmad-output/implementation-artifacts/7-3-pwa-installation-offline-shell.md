...
**Round 1 of 3** — bmad-code-review (Blind Hunter + Edge Case Hunter + Acceptance Auditor, parallel subagents; Opus 4.8 orchestrator) against commit `c7b9c9e` (diff vs `e06092e`; `package-lock.json` + binaries + bmad artifacts excluded → 26 code/test/doc files, ~690 net LOC). **Acceptance Auditor: ALL 7 ACs PASS**, all seven anti-patterns honored, and all five carried deferred items (Task 8) addressed — with every maintainer-approved deviation (placeholder icons, AC5 → installability checklist, pre-existing color-contrast debt) confirmed genuinely documented (not swept). Triage of ~22 raw findings: **0 decision-needed, 1 patch, 6 defer, ~11 dismissed**. Round-guard: Round 1 of the 3-round cap.

- [x] [Review][Patch] `useOnlineStatus` initialised `true`, so launching the installed PWA while offline flashed the full online map (pins/search) for one commit before collapsing to the offline shell [`nextjs-app/hooks/useOnlineStatus.ts:20-22`] — raised by Blind + Edge. **FIXED.** `useState(true)` rendered the online tree once; the mount effect's `sync()` only corrected it on the next commit. Replaced with a lazy initializer `useState(() => typeof navigator !== 'undefined' ? navigator.onLine : true)` so an offline-at-mount device renders the offline shell on the first client commit — safe because the sole consumer (`MapView`) is `ssr:false` and the `typeof` guard keeps it SSR-safe for any future caller; the doc comment was corrected to match. Behavior-only for the offline-at-mount case (online render + forced-state visual path unchanged → no rebaseline). Gate re-run green: `tsc` 0, `eslint` 0, `vitest` 77 files / 665 tests (none dropped).

- [x] [Review][Defer] Offline transition synchronously unmounts the entire venue subtree with no debounce — a transient `navigator.onLine` blip drops in-progress UI state (open detail sheet, selection, scroll, time-slider) and skips overlay exit animations [`nextjs-app/components/custom/map/MapView.tsx:307,737-987`] — deferred *(Target: None — conditional)*, queued in `deferred-work.md`. (blind+edge)
- [x] [Review][Defer] SW `skipWaiting: true` + `clientsClaim: true` can swap the service worker under a live tab across a deploy → Next content-hashed chunks the old tab later requests may be absent from the new precache (`ChunkLoadError`) [`nextjs-app/app/sw.ts:213-214`] — deferred *(Target: None — conditional)*, queued in `deferred-work.md`. (blind)
- [x] [Review][Defer] Offline "cached map background" (AC3) degrades on a true cold offline launch — the MapLibre style/tiles are remote and deliberately NOT runtime-cached, so the style fetch fails → sand fallback + "tile load failed" overlay alongside the "Ingen anslutning" banner [`nextjs-app/app/sw.ts`; `nextjs-app/components/custom/map/MapContainer.tsx`] — deferred *(Target: None — conditional; accepted app-shell-only tradeoff)*, queued in `deferred-work.md`. (edge)
- [x] [Review][Defer] `app/sw.ts` is excluded from the `tsc` gate and esbuild does not type-check it — the file with the trickiest typing (worker globals + `declare global`) has no type enforcement [`nextjs-app/tsconfig.json:42`; `nextjs-app/app/sw.ts`] — deferred *(Target: None — conditional)*; already acknowledged in `deferred-work.md`. (blind)
- [x] [Review][Defer] `inert={tilesFailed}` on the failed map canvas has no paired `aria-hidden` (OnboardingGate sets both) and no fallback below the `inert` baseline (iOS Safari < 15.4) → the dead canvas stays SR/keyboard-reachable on those browsers [`nextjs-app/components/custom/map/MapContainer.tsx:166`] — deferred *(Target: None — conditional; sub-baseline-browser a11y)*, queued in `deferred-work.md`. (blind+edge)
- [x] [Review][Defer] No automated coverage of the real Serwist registration / precache / offline-serving (SW disabled in dev; every offline test uses forced-state or a mocked `navigator.onLine`) — AC2 rests on the manual head/route checks in the Debug Log [`nextjs-app/app/ServiceWorkerProvider.tsx`; `nextjs-app/app/serwist/[path]/route.ts`] — deferred *(Target: None — conditional)*, queued in `deferred-work.md`. (blind)

**Dismissed (verified false positives / by-design / out-of-scope):** cross-tab `storage` handler updates `hasOnboarded` but not `hasReadFlag` — **false positive**, `hasReadFlag` is a one-time mount latch already `true` before any cross-tab `storage` event can arrive (`OnboardingGate.tsx:76-95,148-149`); `storage` `clear()` re-shows onboarding — by-design (a cleared flag means not-onboarded); `VenuePinLayer` unmount orphans MapLibre sources/layers — **false positive**, it manages DOM `Marker`s (not `addSource`/`addLayer`) with full symmetric unmount cleanup, and `MapContainer`/the map instance persist across the toggle (`VenuePinLayer.tsx:307-327`); `spawnSync` git → `randomUUID` revision fallback "fires silently" — acceptable (a random revision still cache-busts; Vercel builds run in a git checkout) and `createSerwistRoute` forces the route static so it evaluates once at build; reconnect MapView test is tautological / AC4 "reload" not asserted — the component test correctly asserts render behavior and reconnect-within-`staleTime` (5 min) intentionally shows cached data via the existing `useVenueSearch` config, which is exactly the "don't bypass the query layer" AC4 contract; OfflineBanner "no `!`" assertion — intentional matter-of-fact-tone guard; axe-mobile four `test.fixme` scans — by-design + documented (pre-existing venue-card contrast debt owned by Story 5.1; the offline-shell scan runs and passes); `manifest.test` does not assert the icon files exist — the four PNGs are committed and verified present on disk; theme-color literals duplicated across manifest/viewport/CSS — inherent (a manifest cannot reference CSS custom properties); only `/` is precached so other routes are offline-unavailable — out of scope (AC3 targets the map shell at `/`; app-shell-only by design).
...
| 2026-06-29 | Code review Round 1 of 3 (bmad-code-review; Opus 4.8 orchestrator). All 7 ACs PASS, all anti-patterns honored. Triage: 0 decision-needed, 1 patch (FIXED), 6 defer, ~11 dismissed. Patch = `useOnlineStatus` offline-launch flash → lazy initializer; gate re-run green (tsc 0, eslint 0, vitest 77/665, no rebaseline). 6 defers queued in `deferred-work.md`. No unresolved HIGH/MEDIUM → Status review → done. |
