...
- [x] [Review][Decision][Med] DISMISSED (won't-fix, human call 2026-07-06: honest pre-existing 11.6 fallback, no fabricated time; AC2 never-fabricate satisfied) — AC2 detail Öppettider row falls back to "Detaljer saknas" / "Details unavailable" rather than omitting the row — on the closed-today / no-hours path `VenueDetailContent.tsx:273` renders `<p>{derivedHours.display ?? labels.detailsUnavailable}</p>`, so the badge is correctly omitted but the Öppettider ROW still shows the "details unavailable" copy. AC2's plain-language "renders NOTHING" and the Design-Gate "omit the line/badge entirely" are, read strictly, not met for the detail row. This is NOT a fabricated closing time (AC2's literal never-fabricate prohibition IS satisfied — no fake "22:00") and it is the pre-existing 11.6 fallback carried verbatim; the deviation is defensible but was never an explicit maintainer decision for this story. [`nextjs-app/components/composed/venue/VenueDetailContent.tsx:273`] Sources: auditor@primary. Recommended: dismiss: the "Detaljer saknas" fallback is the pre-existing 11.6 treatment, honestly signals absent detail, and is not a fabricated close time — AC2's literal never-fabricate prohibition is satisfied; keep the row copy as-is unless the maintainer wants a fully omitted row.
- [x] [Review][Patch][Low] RESOLVED (2026-07-06): rewrote the stale "RED PHASE — why every block is `.skip`-ed" header sections in all 4 un-skipped ATDD files to a "STATUS — GREEN (live, un-skipped)" block that states each spec now runs against the real implementation and gates CI; removed the self-contradicting "GREEN PHASE" inline note in `opening-hours.atdd.test.ts` (folded into the header). Header docs now match the tests' actual green, CI-gating state. [`nextjs-app/test/unit/utils/opening-hours.atdd.test.ts`, `nextjs-app/test/unit/services/venue-store.opening-hours-shape.atdd.test.ts`, `nextjs-app/test/unit/api/venue-detail-route.data-cleanup.atdd.test.ts`, `nextjs-app/test/components/VenueDetailContent.opening-hours-derived.atdd.test.tsx`] — Story-11.9 ATDD files carry stale "RED PHASE — why every block is `.skip`-ed" headers on live, un-skipped GREEN tests — the 4 new ATDD scaffolds were un-skipped and wired to the real implementation this story, but their header boilerplate still claims they are `.skip`-ed RED-phase scaffolds that FAIL against the current tree (one even contradicts itself with a "GREEN PHASE … the pure formatter now exists" line below the RED-phase header). The tests actually run and gate CI, so this is misleading documentation, not a runtime defect — correct/remove the stale RED-phase header comments in these 4 files. [`nextjs-app/test/unit/utils/opening-hours.atdd.test.ts:23`, `nextjs-app/test/unit/services/venue-store.opening-hours-shape.atdd.test.ts:25`, `nextjs-app/test/unit/api/venue-detail-route.data-cleanup.atdd.test.ts:22`, `nextjs-app/test/components/VenueDetailContent.opening-hours-derived.atdd.test.tsx:25`]
- [x] [Review][Defer][Low] AC5 doc + unit tests demonstrate closed-day (`null`) and past-midnight (`close < open`) opening-hours shapes that no seeded launch venue or CI fixture ever uses — all 7 live seed rows and both sunny fixtures open 11:00 and close ≥19:00 on every weekday with no `null` day and no past-midnight row, so the two most spec-emphasised edge shapes ("closed today" + "past-midnight handled") are exercised only in isolation (unit tests + the data-load doc example), never against a live/seeded row a maintainer or the running app renders. Not an AC violation (the seed's job is byte-identical gate-parity; edge shapes are unit-tested). [`nextjs-app/docs/venue-data-load.md:68-77`, `_bmad-output/implementation-artifacts/11-9-venue-data-model-cleanup.sql`] — deferred, pre-existing (verification-coverage gap, not a runtime defect this diff introduced). Sources: auditor@primary.
