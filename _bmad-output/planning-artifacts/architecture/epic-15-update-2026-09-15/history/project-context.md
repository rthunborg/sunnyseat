...
> **Direct-sun truth correction (2026-09-03):** `sunExposurePercent` is only
> clear-sky geometric polygon potential. `directSunState` (`likely`, `blocked`,
> `unknown`) is the authoritative forecast claim. Public amber/recommendations,
> peaks, and windows require `likely`; missing, stale, incomplete, legacy, or
> contradictory weather is `unknown`, never clear. Diffuse daylight is not direct
> sun. This supersedes older `>50% && !weatherGated` shorthand. The persisted
> venue read remains geometry + weather snapshots only; no live Met.no call.
> External-review hardening retains Nowcast provider time, validates every JSON
> slice before use, requires request-relevant evidence for weather provenance and
> full confidence, neutralizes contradictory affirmative DTO tuples, and derives
> detail-window status from the qualifying run rather than the selected instant.
> Follow-up (2026-09-07): malformed present weather booleans retain timestamped
> unknown evidence, preventing substitution by nearby clear slices. Nonempty or
> malformed reasons invalidate likely current/day-series DTOs. Unknown overrides
> legacy obscured copy and ARIA; definite obscured presentation requires blocked.
...
>
> Last updated: 2026-09-03 (direct-sun truth contract and Epic 14 calibration roadmap; Story 13.1 scheduling/resilience remains separate)
>
...
>
> **The app is LIVE on the real data path** (production cutover 2026-06-29: `SUNNYSEAT_VENUE_STORE=supabase`, `SUN_ENGINE=real`, feedback/review persistence = Supabase), and `public.venues` holds the 42-real-venue Göteborg set loaded 2026-07-07. First real-scale contact exposed the Epic 12 launch-readiness gaps; their durable implementation conventions are promoted below where source/test evidence exists. Epic 12 completed its protected closeout on 2026-08-17 with all 14 stories and the epic marked `done`; the formal trace gate is PASS, with strict-cold sample size, provider path granularity, and the absence of a full restore/failover drill recorded as non-blocking NFR concerns.

---

## What Is SunnySeat?

A live Next.js web/PWA app that helps people in Gothenburg find outdoor venue seating in direct sunlight right now. It combines two orthogonal signals into a single honest answer: (1) **geometric sun potential** — NREL SPA solar position + 2.5D building/terrain shadow modelling ("where could the sun reach this seating area?"), and (2) **weather truth** — Met.no cloud + radar-rain data that gates that potential ("but is the sun actually getting through the sky right now?"). Public venue output carries a sun state, sky-condition line, and seating-area sun percentage; confidence remains internal/diagnostic and is not a user-facing score. The server returns a whole-day gated **sun day-series** so the client can scrub the planner time instantly with zero network requests. Epic 12 added launch-readiness foundations for real-scale geometry persistence, live identity/visibility, provider-neutral hours, media, selected-time availability, and operational/dev tooling.
...
| Hosting | Vercel (Fluid Compute, Cron, CDN + Edge proxy; production functions pinned to `dub1`) |
...
- **The real data path is LIVE in production** (cutover 2026-06-29). The full stack — frontend, real sun/shadow engine, Met.no weather, Supabase venue store + feedback/review persistence — runs against live data. Production Vercel functions are pinned to `dub1` so the route runtime stays close to Supabase `eu-west-1`. Epics 1, 2, 3, and 7–11 are shipped history; Epic 12 implementation conventions below are current where promoted, while final review/retro/approval status remains owned by sprint status.
...
- **Story 13.1 launch-resilience posture is active.** The implementation merged through PR #27 and is deployed, but the story remains `in-progress` until provider-classified cold-start evidence, isolated restore evidence, and human device acceptance finish. The security target is exact Next.js `16.3.3` pins for `next`, `eslint-config-next`, and `@next/bundle-analyzer`, with the `@swc/helpers` override preserved and npm 10 lockfile generation required. Launch bundle remediation keeps React Compiler disabled, removes command-menu package weight from the venue search path, and preserves the explicit gates: total emitted static JS <= 614,400 B gzipped, initial route JS <= 286,720 B gzipped, MapLibre async chunk <= 327,680 B gzipped, plus separate all-routes MapLibre async verification. Production weather snapshots still expire honestly to `unknown` after two hours; a 2026-09-03 audit found repeated GitHub scheduled-delivery gaps beyond that TTL, so an independent authenticated scheduler with a durable cross-scheduler lease is required before live-weather readiness can be called reliable. Do not extend the TTL or reintroduce request-path Met.no calls.
...
| Epic-12 Retro Notes | `_bmad-output/auto-bmad/retro-notes/epic-12.md` |
...
- SunnySeat computes two orthogonal facts and combines them only at the public-verdict boundary:
  1. **Geometric sun potential** — NREL SPA + shadow/terrain geometry produce `sunExposurePercent` and a geometric status answering “could direct sun reach this seating polygon under a clear sky?” The percentage never becomes a probability or a weather-adjusted percentage.
  2. **Predicted direct sun** — the server-owned classifier combines that geometry with fresh Met.no snapshot evidence and emits `directSunState: likely | blocked | unknown` plus safe reasons. Bright diffuse daylight is never direct sun.
- **The public predicate:** amber, “soligt”, partner sunny badges, “Mest sol” priority, public sun windows, and peaks require geometry above 50% **and** `directSunState === 'likely'`. Among likely venues, geometric percentage remains the ranking value. Missing legacy direct state normalizes to `unknown`; `weatherGateState` is compatibility output and is not authoritative.
- **The classifier (`lib/services/direct-sun-classifier.ts`):** below-horizon/geometry ≤50% is blocked by geometry; malformed geometric evidence is unknown. For positive geometry, known precipitation, fog/cloudy symbols, fog/raw/effective obstruction ≥80%, or positive precipitation blocks. Strictly intermediate obstruction, broken-cloud symbols, incomplete/malformed/unmatched/contradictory evidence is unknown. `likely` requires valid total plus all layer/fog fields ≤20%, an explicit zero next-hour forecast precipitation amount, and a coherent clear/fair symbol; `isRaining: false` alone cannot fill missing forecast precipitation evidence.
- **Temporal admission precedes classification:** persisted public reads require an unexpired two-hour snapshot and select by valid provider `validAt` within ±90 minutes; the convenience `minutes` key never bypasses this check. The pure classifier intentionally owns weather semantics only and must receive evidence admitted by that boundary.
- **Effective and raw cover:** effective obstruction remains `clamp(1.0*low + 1.0*medium + 0.25*high, 0, 100)`, but the conservative launch policy also checks raw total. Therefore the former Epic 10 “100% high-only cloud is sunny” shortcut is superseded: it is blocked initially when raw total is ≥80, pending field calibration or an irradiance-informed model. Never silently change weights or thresholds.
- **Precipitation precedence:** either a positive Locationforecast next-hour amount/symbol or a positive near-now Nowcast radar rate is a one-way blocker. No rain alone is never positive sun evidence. Sky presentation is projected from the same evidence so rain/cloud/fog cannot coexist with a contradictory “clear” label.

### `undefined` never `0` — the missing-weather discipline (Stories 10.1 / 10.3 / 10.4)

- **A missing weather field stays `undefined`, never coerced to `0`.** For positive geometry, any missing required classifier evidence is `directSunState: unknown`; it is neither non-gated sunny nor clear. An omitted Nowcast rate contributes no rain blocker but cannot independently prove sun. **Never `?? 0` a cloud, fog, precipitation, or rain field.** Weather-unavailable presentation uses no fabricated sky descriptor and may expose only explicitly labelled clear-sky geometry.
...
- **Resolved inert-guard debt (Story 11.7):** the orphaned `toSunStatusToken` mapper (`lib/utils/sun-status-presentation.ts`) — advertised as the single exhaustive DTO→token predicate but consumed by no render surface (every surface branches inline) — was **deleted** in Story 11.7 rather than wired in (wiring ~7 surfaces would touch visuals). The `never`-exhaustive compile-time guard over `VenueSunStatus` survives via the sibling `windowLabelTier` switch in the same module. Historical raw-status branching is superseded: obscured surfaces require `directSunState === 'blocked'` as well as `CloudObscured`; unknown always takes precedence.
...
- **E2E weather-boundary specs mock at the DTO boundary with `page.route`, NOT via `?_state` or `?venue=`.** The working seam (`test/e2e/epic-10-weather-matrix.spec.ts`) intercepts list and detail DTOs with explicit `directSunState`, `directSunReasons`, compatibility status/gate, and sky context for clear, broken-cloud, overcast, precipitation, fog, stale/missing, incomplete, and contradictory cases. It blocks `api.met.no` requests, drives the real card/pin/detail UI, and runs at both breakpoints. Missing weather omits the sky descriptor; the pure classifier and persisted-route layers are tested below the DTO seam.
...
- **`applyCloudGate` is legacy compatibility logic, not the public truth boundary.** New or modified production paths must call the direct-sun classifier and its status/gate/sky projections; do not add `applyCloudGate` callers. Its optional rain parameter remains only to preserve historical pure-helper coverage until that API is deliberately removed.
...
- **The server emits a whole-day direct-sun `sunDaySeries`; the client derives every time-dependent surface from it and NEVER re-gates.** `directSunState` is authoritative: public sunny is `sunExposurePercent > 50 && directSunState === 'likely'`. Missing state is `unknown`, never likely. Geometry is clear-sky potential; blocked/unknown are non-amber and unknown must carry a qualified potential label.
...
- **Orphaned inert-guard mappers get DELETED, not wired in.** The `toSunStatusToken` mapper (`lib/utils/sun-status-presentation.ts`) — advertised as an exhaustive DTO→token predicate but consumed by no render surface (historically surfaces branched on raw status; the direct-sun contract now requires `blocked` before obscured presentation) — was deleted; wiring it into ~7 surfaces would have broken the byte-identical design gate. The `never`-exhaustive `VenueSunStatus` compile-time guard survives via the sibling `windowLabelTier` switch in the same module. (Mirrored in the Epic-10 exhaustiveness note above.)
...
- **Story 12.14 superseded the old minute-imprecise display path for selected-time surfaces.** Use `getVenueAvailabilityAt` / `formatOpeningHoursAt` for selected-instant open/closed decisions and copy. The legacy `formatOpeningHours` helper still derives from the current Stockholm weekday only; do not use it for planner-selected availability.
...
## Epic 12 Ratified Conventions

Durable patterns promoted from Epic 12 ("Real-Venue Launch Readiness" — performance, trust, hours, media, and operations). These are **binding** for stories that touch the named seams. Full rationale and caveats live in `_bmad-output/auto-bmad/retro-notes/epic-12.md`, story files, and the deferred queue.

### Provider-neutral hours and provenance (Story 12.1)

- **Do not implement weekly Google Places `regularOpeningHours` synchronization.** Under the completed EEA/MapLibre policy research, Google hours and Google-returned/provider URLs must not enter Supabase, durable queues, logs, fixtures, DTOs, or public UI. Only Google Place IDs may be retained as server-only identifiers.
- **Canonical hours come from independently permitted evidence.** Unsupported split, 24/7, seasonal, or holiday-specific schedules route the whole venue to manual review rather than being flattened or guessed. OSM remains non-writing supplemental research until coverage and ODbL attribution/derivative/share-alike gates are explicitly approved.

### Persisted real-scale sun geometry (Story 12.3)

- **Persisted day-series reads use the canonical `geometry_input_hash` (`g1:<64 lowercase hex>`), not ad-hoc local approximations.** The source seam is `lib/services/sun-geometry-hash.ts`; repository reads must match `venue_geometry_inputs.current_geometry_input_hash`, `stockholm_date`, and `venue_sun_geometry_series.geometry_input_hash`.
- **The public `/api/venues` persisted-read contract is three Supabase calls for the full list:** one public-venue projection query (`venues` filtered by `hidden=false` and `deleted_at is null`), one service-role `read_current_venue_sun_geometry_batch` RPC, and one deduplicated `weather_bucket_snapshots` query for current coordinate buckets. Do not reintroduce per-venue REST fan-out, `geometry_inputs`/`venue_sun_geometry_series` client-side joins, live Met.no calls, caster/building recomputation, or other provider fallback on this route.
- **`read_current_venue_sun_geometry_batch(text[], date)` is service-only and atomic.** It is `security definer`, `stable`, revoked from `public`/`anon`/`authenticated`, granted only to `service_role`, and returns each requested venue's current input status/hash plus exact date/hash coverage in request order. Keep it as the single batch lookup seam for public persisted geometry.
- **Public requests read persisted deterministic geometry and read-time weather snapshots, then fail closed.** Missing exact current coverage, a non-ready current input, date/hash mismatch, or malformed persisted gate data must return the existing coverage-missing/error path or sanitized non-sunny output; do not silently fall back to stale persisted coverage, live recomputation, weather/provider calls, or seed/fixture data in production public list/detail routes.
- **Keep production functions colocated with Supabase.** `nextjs-app/vercel.json` pins `regions: ["dub1"]`; preserve this unless the Supabase project leaves `eu-west-1` or latency evidence justifies a coordinated move. A region change is a data-path architecture change, not a cosmetic deployment tweak.

### Public venue identity and visibility (Stories 12.5 / 12.7 / 12.10 / 12.14)

- **Use one server-only public resolver/visibility guard.** Public list, ID/slug detail, reviews GET/POST, feedback POST, favourites-by-ID, and detail-prefetch paths must resolve live identity through `resolvePublicVenueIdentifier` / the store's public projection and reject unknown, `hidden=true`, or `deleted_at` venues consistently. Fixture fallback remains fixture-mode-only.
- **`hidden` is the canonical live visibility column.** Do not reintroduce invented `visibility`, `is_hidden`, or route-local vocabularies. Hidden venues may still be included in service-role precompute targets where needed for operational completeness, but never leak into public route DTOs.
- **Display coordinates are separate from engine/weather coordinates.** Public `location` may come from `display_lat/display_lng`; server-only `engineLocation` remains the persisted `lat/lng` used for weather and geometry fallbacks.

### Public sun presentation (Stories 12.2 / 12.6 / 12.8 / 12.13)

- **One public-sunny predicate:** `isVenuePubliclySunny` in `lib/utils/public-sun.ts` is the shared source for amber versus neutral presentation and sorting. Public sunny means `sunExposurePercent > 50 && directSunState === 'likely'`; raw `VenueSunStatus` and legacy weather-gate state are diagnostic only.
- **Grey map pins are percentage-free.** Shade/cloud/not-sunny pins use the single grey no-number presentation; `CloudObscured` permits diagnostic copy in cards/detail/quick-info only with `directSunState === 'blocked'`; unknown takes precedence for visible copy and accessible names.
- **User-facing confidence indicators are removed.** Keep confidence as internal/diagnostic data only. Public About copy and UI must not invent accuracy percentages or expose confidence badges without measured aggregate evidence.
- **Fail-closed weather provenance must travel with DTOs/day-series.** Do not infer `weatherGateState` later from sky strings or `CloudObscured`; semantically contradictory tuples are sanitized before public presentation.

### Selected-instant availability (Story 12.14)

- **One selected instant drives hours eligibility and hours copy.** Use `getVenueAvailabilityAt` / `formatOpeningHoursAt` with the planner-selected instant. Map/ranked discovery rows, pins, and availability counts hide explicitly closed venues.
- **Closed exact-name search and saved favourites remain inspectable.** Exact by-name search can return a closed venue labelled `Stängt vid vald tid` and open detail; favourites keep saved closed venues as greyed accessible rows with enabled detail navigation. Closed favourites do not regain map pins. Unknown-hours venues remain visible without open/closed claims.
- **The opening-hours data shape remains single-interval-per-weekday.** Numeric ISO weekday keys `"1"`..`"7"`; missing/`null` weekday = explicitly closed; whole field absent = unknown; `close < open` = prior-day past-midnight spillover.

### Detail prefetch and request-count guard (Story 12.10)

- **Same-date scrub still issues zero venue-list requests.** Bounded detail prefetch must not restart on same-date scrubs or silently violate the Epic 11 request-count gate.
- **The detail prefetcher is bounded and cancellable:** initial-settle only, at most six candidates, concurrency two, exact TanStack detail query keys shared with `useVenueDetail`, in-flight selected detail preserved when opened, and failures enter cooldown instead of retry storms.

### Media and dev-only operations (Stories 12.5 / 12.12)

- **Venue media uses Supabase public object URLs under `venue-media/{slug}/{mediaVersion}/{card|hero}.webp`.** Validate origin, slug, media version, exact path shape, rendition filename, and no query/hash before rendering. Cards prefer `cardUrl`; detail heroes prefer `heroUrl`; fallback placeholders must render without broken images.
- **The venue editor is dev/localhost-only with production hard deny.** Routes under `app/api/dev/venues` and editor store paths must deny production before considering flags/input. The editor may mutate display coordinates, hidden state, media metadata, and geometry-publish seams only through the scoped dev API; it is not a production admin surface.

### Coach guide, row sheet, and visual-gate discipline (Stories 12.9 / 12.11 / 12.12 / 12.14)

- **First-run coach marks are real product UI.** Forced states `coach-mark-first` and `coach-mark-middle` map to the first guide steps; Settings relaunch, first-run seen-state, focus trap, reduced motion, Swedish copy, and both desktop/mobile axe lanes are part of the contract.
- **The mobile venue sheet is row-count based.** `map-panel-venues` normally captures `N=3` complete rows; extra dev/test variants may use `_sheetRows=0|1|3|max` or `_sheetDrag=mid`. Do not revert to the old fixed `mid` snap semantics.
- **Visual references still require explicit provenance.** `screen_id:` markers must be explicit in stories for `story-review.sh`; visual-provider credential gaps do not authorize reference replacement, manual-pass claims, or sprint-status transitions. Any reference promotion/rebaseline updates `REBASELINE-LOG.md`.
...
| coach-mark-first           | `/?_state=coach-mark-first&_time=14:00`                     | mobile   | First guide step open, both public pin presentations explained in the coach card, focus on guide heading, skip/close visible. |
| coach-mark-first           | `/?_state=coach-mark-first&_time=16:30`                     | desktop  | Same first guide step on desktop layout.                                                    |
| coach-mark-middle          | `/?_state=coach-mark-middle&_time=14:00`                    | mobile   | Middle guide step anchored to the mounted mobile time/date planner, with row-count sheet and controls stable. |
...
| map-panel-venues           | `/?_state=map-panel-venues&_time=14:00`                     | mobile   | Row-count venue sheet at `N=3` complete compact rows, clamped by available venues/max height; `_time` pins the MVP reference time. Dev/test capture variants may add `_sheetRows=0\|1\|3\|max` or `_sheetDrag=mid`. |
| map-with-selected-venue    | `/?venue=test-venue-sunny&_state=map-with-selected-venue&_time=14:00` | mobile   | Selected pin emphasis without a third data shape; composition awaits the Story 12.6 rebaseline. |
| map-with-obscured-venue    | `/?_state=map-with-obscured-venue&_time=14:00`             | mobile   | Story 12.6/10.2 weather-gated obscured surface — pins are the single grey no-number cloud presentation while selected-venue quick-info/detail copy still exposes `directSunState: 'blocked'` + `CloudObscured` + `skyCondition: 'overcast'` as "Sol bakom moln" diagnostics. No live Met.no; reachable on the fixture/CI path. Auto-selects the first venue like `map-with-selected-venue`. |
| map-with-obscured-venue    | `/?_state=map-with-obscured-venue&_time=14:00`             | desktop  | Same obscured grey-pin surface on desktop viewport.                                         |
| map-selected-time-open     | `/?_state=map-selected-time-open&_time=14:00`              | mobile   | Story 12.14 selected-instant open reference — open/unknown discovery rows, counts, tags, and pins remain visible at the selected planner time. Implementation-derived active PNG requires maintainer approval before promotion. |
| map-selected-time-open     | `/?_state=map-selected-time-open&_time=16:30`              | desktop  | Same selected-instant open reference on desktop. Implementation-derived active PNG requires maintainer approval before promotion. |
| map-selected-time-closed   | `/?_state=map-selected-time-closed&_time=09:00&_search=Kaf%C3%A9%20Magasinet` | mobile   | Story 12.14 selected-instant closed reference — dev forcing seeds an exact closed search for `Kafé Magasinet`; the `11:00-22:00` venue is retained as a labelled exact result while discovery rows/counts/pins exclude it at 09:00. Implementation-derived active PNG requires maintainer approval before promotion. |
| map-selected-time-closed   | `/?_state=map-selected-time-closed&_time=09:00&_search=Kaf%C3%A9%20Magasinet` | desktop  | Same exact closed search state on desktop; the retained search row is inspectable but the closed venue remains absent from discovery rows/counts/pins at 09:00. Implementation-derived active PNG requires maintainer approval before promotion. |
...
| venue-detail-obscured      | `/?venue=test-venue-sunny&_state=venue-detail-obscured&_time=14:00` | mobile   | Story 10.2: the venue-detail surface with the seeded venue in the weather-blocked `directSunState: 'blocked'` + `CloudObscured` + `overcast` state (muted hero badge/headline + sky line; the sun timeline stays as clear-sky potential). No live Met.no. |
...
| venue-photo-loaded         | `/?venue=test-venue-sunny&_state=venue-photo-loaded&_time=14:00` | mobile   | Story 12.12 approved implementation-derived reference: seeded venue detail uses Supabase `venue-media` card/hero renditions; DOM/E2E separately proves list card and mobile anchored QuickInfo placeholder. Promoted from the approved `20260719T195547` candidate set. |
| venue-photo-loaded         | `/?venue=test-venue-sunny&_state=venue-photo-loaded&_time=16:30` | desktop  | Same photo-loaded seeded venue on desktop; DOM/E2E separately proves list card, desktop QuickInfo card rendition, and detail hero rendition. Promoted from the approved `20260719T195547` candidate set. |
| venue-photo-fallback       | `/?venue=test-venue-sunny&_state=venue-photo-fallback&_time=14:00` | mobile   | Story 12.12 approved fallback reference: seeded venue points at missing `venue-media` renditions so card/detail fall back to branded initials/placeholder without broken images. Promoted from the approved `20260719T195547` candidate set. |
| venue-photo-fallback       | `/?venue=test-venue-sunny&_state=venue-photo-fallback&_time=16:30` | desktop  | Same photo-fallback seeded venue on desktop; DOM/E2E separately proves list, QuickInfo, and detail fallback surfaces. Promoted from the approved `20260719T195547` candidate set. |
...
## Approved seasonal geometry direction — 2026-09-10 (not implemented)
...
Rasmus accepted the [Epic 15 owner policy](_bmad-output/planning-artifacts/decisions/epic-15-owner-policy-2026-09-10.md). This supersedes earlier pending product-choice wording for NFR20/NFR35/NFR40 and E15-AD-01/02; it does not certify measurements or deployed behavior.
