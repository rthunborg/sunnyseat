...
- [Phase 5 â€” dev-story] Supabase MCP apply_migration/execute_sql unavailable in delegate sessions (OAuth not active); live-apply worked via local supabase/postgres Docker image + SUPABASE_DB_POOLER_URL from gitignored .env.local. Future migration steps: confirm MCP auth up front or expect the Docker-psql fallback.
- [Phase 5 â€” dev-story] Quick-info opening-hours now locale-derived (EN shows 'Open until 22:00') â€” genuine i18n improvement over the old raw-Swedish string a prior story comment called non-localizing.
- [Phase 8 â€” nfr] 11.9 was the epic's first post-epic-end NFR-surface expansion (live prod-DB migration on a story folded in from dissolved Epic 12) â€” re-running the NFR delta instead of trusting the frozen epic-end PASS proved worthwhile; live-DB posture (RLS/deny-by-default) is only verifiable against the running DB, not source.
- [Phase 8 â€” retro] Supabase-MCP-unavailable -> Docker-psql fallback recurred (2nd time) â€” pre-flight Docker-psql credentials before any future live-migration story.
